...
def populatebyrow(prob, graphList):
    ...
    ###Find 'Z' variable list.###
    for graph in graphList:
        g = graph.graphID
        for k in graph.messageNodes:
            childNode = graph.childNodes[k][0]
            for r in range(1, numberOfProcessor + 1):
                for t in range(graph.asap[childNode], graph.alap[childNode] + 1): 
                    myString = "z"+(str)(k)+","+(str)(r)+","+(str)(t)
                    zVariables.append(myString)
    ...
    ###Unique Start time of tasks and messages###
    ...
    for graph in graphList:
        ...
        for k in graph.messageNodes:
            parentNode = graph.parentNodes[k][0]
            childNode = graph.childNodes[k][0]
            rowName = "Equation eq3 (Message"+(str)(k)+")"
            variables = []
            coefficients = []
            for r in range(1, numberOfProcessor + 1):
                for t1 in range(graph.asap[parentNode], graph.alap[parentNode] + 1): 
    ...
    ###Equation 9 (Dependency Constraints).###
    for graph in graphList:
        g = graph.graphID
        for k in graph.messageNodes:
            rowName = "Dependency Constraints eq9 (Message"+(str)(k)+")"
            parentNode = graph.parentNodes[k][0]
            childNode = graph.childNodes[k][0]
            variables = []
            coefficients = []
    ...
    ###Equation 10, 11 & 12 (Linearization of equation 9).###
    for graph in graphList:
        g = graph.graphID
        for k in graph.messageNodes:
            childNode = graph.childNodes[k][0]
            rowName = "Linearization eq10 (Message"+(str)(k)+")"
            rowName1 = "Linearization eq11 (Message"+(str)(k)+")"
            rowName2 = "Linearization eq12 (Message"+(str)(k)+")"
            tempRows = []
            tempRowNames = []
            tempRhs = []
            tempSense = "" 
    ...
    for graph in graphList:
        g = graph.graphID
        for k in graph.messageNodes:
            rowName = "Precedence Constraints eq13 (Message"+(str)(k)+")"
            childNode = graph.childNodes[k][0]
            variables = []
            coefficients = []
...
f = open("./outputFiles/output_ILP1.txt", "a")
