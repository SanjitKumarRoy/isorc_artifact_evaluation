...
def populatebyrow(prob, graphList):
    ...
    ###Equation 3 & 4 (Resource Constraints)###
    ##Resource Constraints for Processors.##
    for r in range(1, numberOfProcessor + 1):
        for t in range(1, hyperPeriod + 1):
            rowName = "Resource Constraints eq3 (Processor) Time"+(str)(t)+":"
            variables = []
            coefficients = []
            for graph in graphList:
                g = graph.graphID
    ...
    for r in range(1, numberOfBus + 1):
        for t in range(1, hyperPeriod + 1):
            rowName = "Resource Constraints eq4 (Bus) Time"+(str)(t)+":"
            variables = []
            coefficients = []
            for graph in graphList:
                g = graph.graphID
    ...
    ###Equation 11 (Linearization of equation 7).###
    for graph in graphList:
        g = graph.graphID
        for k in graph.messageNodes:
            parentNode = graph.parentNodes[k][0]
            childNode = graph.childNodes[k][0]
            rowName = "Linearization eq11 (Message"+(str)(k)+")"
            variables = []
            coefficients = []
            for r in range(1, numberOfProcessor + 1):
                for t1 in range(graph.asap[parentNode], graph.alap[parentNode] + 1): 
    ...
    ###Equation 13, 14 & 15 (Linearization of equation 7).###
    for graph in graphList:
        g = graph.graphID
        for k in graph.messageNodes:
            parentNode = graph.parentNodes[k][0]
            childNode = graph.childNodes[k][0]
            rowName = "Linearization eq13 (Message"+(str)(k)+")"
            rowName1 = "Linearization eq14 (Message"+(str)(k)+")"
            rowName2 = "Linearization eq15 (Message"+(str)(k)+")"
            tempRows = []
            tempRowNames = []
            tempRhs = []
            tempSense = "" 
    ...
    ###Equation 8 (Precedente Constraints).###
    for graph in graphList:
        g = graph.graphID
        for k in graph.messageNodes:
            rowName = "Precedence Constraints eq8 (Message"+(str)(k)+")"
            parentNode = graph.parentNodes[k][0]
            childNode = graph.childNodes[k][0]
            variables = []
            coefficients = []
    ...
    ###Equation 9 (Precedente Constraints).###
    for graph in graphList:
        g = graph.graphID
        for k in graph.messageNodes:
            rowName = "Precedence Constraints eq9 (Message"+(str)(k)+")"
            parentNode = graph.parentNodes[k][0]
            childNode = graph.childNodes[k][0]
            variables = []
            coefficients = []
    ...
    ###Equation 10 (Precedence Constraints).###
    for graph in graphList:
        g = graph.graphID
        for k in graph.messageNodes:
            rowName = "Precedence Constraints eq10 (Message"+(str)(k)+")"
            childNode = graph.childNodes[k][0]
            variables = []
            coefficients = []
...
f = open("./outputFiles/output.txt", "a")
